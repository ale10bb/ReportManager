...
def read_document(work_path: str) -> Attachment:
    ...
    document_paths = file_paths(filtered_walk(
        work_path, included_files=['*.doc', '*.docx'], excluded_files=['~$*', '*XT13*', '*签发意见单*']
    ))
    ...
    for document_path in document_paths:
        ...
        try:
            ...
            for i in range(5):
                re_result = re.search(
                    pattern, document.Paragraphs(i+1).Range.Text)
                if re_result:
...
def gen_XT13(authorname: str, code: str, project_name: str, target_path):
    ...
    try:
        ...
        if 'PRO' in code or 'PST' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-PROPST.docx'))
        elif 'SOF' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-SOF.docx'))
        elif 'DSYS' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-DSYS.docx'))
        elif 'SRV' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-SRV.docx'))
        elif 'PER' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-PER.docx'))
        elif 'FUN' in code:
            template_document = Document(os.path.join(
                'template', 'RD-XT13测评报告审核、签发意见单-FUN.docx'))
        else:
            # 目前无视其他类型的报告
            return
        # 项目编号(bold)
