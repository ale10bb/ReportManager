...
class Dingtalk:
    ...
    def __init__(self, chatbot:dict={}, chatbot_debug:dict={}, attend_url:str='', interaction_url:str='', enabled:bool = False):
        ''' 初始化dingtalk的配置
        ...
        if chatbot['webhook'] and chatbot['secret']:
            self._chatbot = DingtalkChatbot(webhook=chatbot['webhook'], secret=chatbot['secret'])
        ...
        if chatbot_debug['webhook'] and chatbot_debug['secret']:
            self._chatbot_debug = DingtalkChatbot(webhook=chatbot_debug['webhook'], secret=chatbot_debug['secret'])
    ...
    def send_markdown(self, title:str, content:str, phone:str='', to_debug:bool=False, to_stdout:bool=False):
        ...
        if not self._enabled or (to_debug and not self._chatbot_debug) or (not to_debug and not self._chatbot):
            to_stdout = True
        if to_stdout:
        ...
        if to_debug:
            r = self._chatbot_debug.send_markdown(title=title, text=content)
        else:
            if phone:
                logger.debug('at_mobiles: True')
                r = self._chatbot.send_markdown(
                    title=title + '@' + phone, 
                    text=content, 
                    at_mobiles=[phone], 
                    is_auto_at=False
                ...
                # 不指定at时直接发送
                r = self._chatbot.send_markdown(title=title, text=content)
        logger.debug(r)
    ...
    def send_action_card(self, content:str, to_debug:bool=False, to_stdout:bool=False):
        ...
        if to_debug:
            if buttons:
                r = self._chatbot_debug.send_action_card(
                    ActionCard(
                        title='任务状态', 
                        text=content,
                        btns=buttons,
                ...
                r = self._chatbot_debug.send_markdown(
                    title='任务状态', 
                    text=content
                )
        else:
            if buttons:
                r = self._chatbot.send_action_card(
                    ActionCard(
                        title='任务状态', 
                        text=content,
                        btns=buttons,
