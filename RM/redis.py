...
class RedisStream:
    ...
    def add(self, source: str, name: Literal['receive', 'read', 'resend'], fields: dict = None) -> str:
        ''' 在Stream中插入一条指令
    ...
    def trim(self):
        ...
        '''
        logger = logging.getLogger(__name__)
        logger.debug('receive original len: %s',
                     self._r.xtrim(name='receive', maxlen=10))
        logger.debug('read original len: %s',
                     self._r.xtrim(name='read', maxlen=10))
        logger.debug('resend original len: %s',
                     self._r.xtrim(name='resend', maxlen=10))
