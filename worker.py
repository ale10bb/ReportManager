...
def do_mail(parsed_mail: Parsed_Mail):
    ...
    logger.info('reading "%s" from "%s"',
                parsed_mail['operator'], parsed_mail['from_'])
...
def handle_submit(work_path: str, content: Content, attachment: Attachment, warnings: list[str]):
...
def do_resend(id: str | int, redirect: str = ''):
    ...
    work_path = list(dir_paths(filtered_walk(
        os.path.join(storage, 'temp' if isinstance(
            record['id'], str) else 'archive'),
